  ...
  async getTicketsReadyForSpotting(req, res) {
    try {
      ...
      res.status(200).json({
        ...
        criteria: 'comment7d is NULL, empty, or contains TK - PERMIT EXTENDED, TK - LAYOUT, or TK - LAY OUT, and no endingDate for SPOTTING status. Excludes tickets with comment7d containing: TK - CANCELLED, TK - HOLD OFF, TK- ON HOLD OFF, TK - ON HOLD OFF, TK - COMPLETED, TK - COMPLETE, COMPLETED, COMPLETE, TK - EXPIRED, TK - NEEDS PERMIT EXTENSION',
  ...
  async getTicketsReadyForConcrete(req, res) {
    try {
      ...
      res.status(200).json({
        ...
        type: 'CONCRETE',
        count: ticketsWithAddresses.length,
        criteria: 'SPOTTING completed (has endingDate) and has SAWCUT status. Includes tickets with comment7d containing: TK - PERMIT EXTENDED, TK - LAYOUT, TK - LAY OUT, TK - ON PROGRESS, TK- ON PROGRESS (allowing text before and after keywords). Excludes tickets with comment7d containing: TK - CANCELLED, TK - HOLD OFF, TK - ON HOLD OFF, TK- ON HOLD OFF, TK - COMPLETED, TK - COMPLETE, COMPLETED, COMPLETE, TK - EXPIRED, TK - NEEDS PERMIT EXTENSION',
        tickets: ticketsWithAddresses
      });
    } catch (error) {
  ...
  async getTicketsReadyForAsphalt(req, res) {
    try {
      ...
      res.status(200).json({
        ...
        type: 'ASPHALT',
        count: ticketsWithAddresses.length,
        criteria: 'SPOTTING completed and either has GRINDING status (no SAWCUT) OR concrete Pour phase completed. Includes tickets with comment7d containing: TK - ON PROGRESS, TK - ON LAYOUT, TK - LAYOUT, TK - LAY OUT, TK- ON PROGRESS, TK- ON LAYOUT, TK- LAYOUT (allowing text before and after keywords). Excludes tickets with comment7d containing: TK - CANCELLED, TK - HOLD OFF, TK- ON HOLD OFF, TK - ON HOLD OFF, TK - COMPLETED, TK - COMPLETE, COMPLETED, COMPLETE, TK - EXPIRED, TK - NEEDS PERMIT EXTENSION',
        tickets: ticketsWithAddresses
      });
    } catch (error) {
