...
class IncidentsMx {
  ...
  static async findById(incidentId) {
    const res = await db.query('SELECT * FROM IncidentsMx WHERE incidentId = $1;', [incidentId]);
    return res.rows[0];
  }

  static async findAll() {
    const res = await db.query('SELECT * FROM IncidentsMx;');
    return res.rows;
  }

  static async update(incidentId, name, earliestRptDate, updatedBy) {
    const res = await db.query(
      'UPDATE IncidentsMx SET name = $1, earliestRptDate = $2, updatedAt = CURRENT_TIMESTAMP, updatedBy = $3 WHERE incidentId = $4 RETURNING *;',
  ...
  static async delete(incidentId) {
    const res = await db.query('UPDATE IncidentsMx SET deletedAt = CURRENT_TIMESTAMP WHERE incidentId = $1 RETURNING *;', [incidentId]);
    return res.rows[0];
  }
}
