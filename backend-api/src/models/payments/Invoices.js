...
class Invoices {
  ...
  static async findById(invoiceId) {
    const res = await db.query('SELECT * FROM Invoices WHERE invoiceId = $1;', [invoiceId]);
    return res.rows[0];
  }

  static async findAll() {
    const res = await db.query('SELECT * FROM Invoices;');
    return res.rows;
  }

  static async update(invoiceId, ticketId, invoiceNumber, invoiceDateRequested, amountRequested, status, invoiceURL, updatedBy) {
    const res = await db.query(
      'UPDATE Invoices SET ticketId = $1, invoiceNumber = $2, invoiceDateRequested = $3, amountRequested = $4, status = $5, invoiceURL = $6, updatedAt = CURRENT_TIMESTAMP, updatedBy = $7 WHERE invoiceId = $8 RETURNING *;',
  ...
  static async delete(invoiceId) {
    const res = await db.query('UPDATE Invoices SET deletedAt = CURRENT_TIMESTAMP WHERE invoiceId = $1 RETURNING *;', [invoiceId]);
    return res.rows[0];
  }
}
