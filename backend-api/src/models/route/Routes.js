...
class Routes {
  ...
  static async findById(routeId) {
    const res = await db.query('SELECT * FROM Routes WHERE routeId = $1;', [routeId]);
    return res.rows[0];
  }
  ...
  static async update(routeId, routeCode, type, startDate, endDate, updatedBy) {
    const res = await db.query(
      'UPDATE Routes SET routeCode = $1, type = $2, startDate = $3, endDate = $4, updatedAt = CURRENT_TIMESTAMP, updatedBy = $5 WHERE routeId = $6 RETURNING *;',
  ...
  static async delete(routeId) {
    const res = await db.query('UPDATE Routes SET deletedAt = CURRENT_TIMESTAMP WHERE routeId = $1 RETURNING *;', [routeId]);
    return res.rows[0];
  }
}
